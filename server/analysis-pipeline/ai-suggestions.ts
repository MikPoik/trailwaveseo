...
const seoSuggestionsCache = new Map<
  string,
  ...
  try {
    ...
    const businessContext = siteOverview
      ? {
          ...
          contentType:
            pageData.paragraphs && pageData.paragraphs.length > 8
              ? "Long-form content"
              : "Short-form content",
        }
      : {
      ...
      ${siteStructure.allPages
        ...
        .map((page: any) => {
          const pageKeywords = extractKeywords(
            [
              page.title || "",
              page.headings.find((h: any) => h.level === 1)?.text || "",
            ].join(" "),
          );
          const commonKeywords = extractedKeywords.filter((keyword) =>
            pageKeywords.includes(keyword),
          );
          const isAlreadyLinked =
            pageData.internalLinks?.some((link: any) =>
        ...
        .map(
          (item) =>
            `- "${item.page.title || "Untitled"}" (${item.page.url}) - Related topics: ${item.commonKeywords.slice(0, 3).join(", ")} ${item.isAlreadyLinked ? "[Already linked]" : "[Link opportunity]"}`,
        )
        .join("\n")}
    ...
    console.log(
      `CTA Elements for ${pageUrl}:`,
      pageData.ctaElements ? pageData.ctaElements.length : "undefined",
    );
...
`
    ...
    const ctaElementsWithText = pageData.ctaElements
      ? pageData.ctaElements.filter(
          (cta: any) => cta.text && cta.text.trim() !== "",
        )
      : [];
    ...
    const isFormType = (type: string) =>
      ...
      formTypes.some((ft) => type.startsWith(ft + "_with_captcha"));

    const forms = ctaElementsWithText.filter((cta: any) =>
      isFormType(cta.type),
    );
    const linkButtons = ctaElementsWithText.filter(
      (cta: any) => cta.type === "link_button",
    );
    const inputButtons = ctaElementsWithText.filter(
      (cta: any) => cta.type === "input_button",
    );
    const regularButtons = ctaElementsWithText.filter(
      (cta: any) => cta.type === "button",
    );

    // Calculate total buttons (excluding forms) - for all CTAs, not just those with text
    const allForms = pageData.ctaElements
      ? pageData.ctaElements.filter((cta: any) => isFormType(cta.type))
      : [];
    const allButtons = pageData.ctaElements
      ? pageData.ctaElements.filter(
          (cta: any) =>
            cta.type === "button" ||
            cta.type === "input_button" ||
            cta.type === "link_button",
        )
      : [];

    // Analyze CTA elements on the page
    const ctaAnalysis =
      ctaElementsWithText && ctaElementsWithText.length > 0
        ? `
...
${
  forms
    .map((cta: any) => {
      const hasCaptcha =
        cta.type.includes("_with_captcha") ||
        (cta.attributes && cta.attributes.captcha === "detected");
...
Respond in JSON: {"suggestions": ["suggestion 1", "suggestion 2", ...]}`;
    ...
    if (response && response.choices[0].finish_reason === "length") {
      console.warn(
        `Response was truncated for ${url} - may need to increase max_completion_tokens`,
      );
    }

    if (!response) {
      console.error(
        `All ${maxRetries} attempts failed for ${url}. Last error:`,
        lastError,
      );
      return [];
    }
    ...
    try {
      ...
      console.warn(
        `Initial JSON parse failed for ${url}, attempting cleanup...`,
      );
      ...
      try {
        ...
        console.error(
          `Failed to parse JSON even after cleanup for ${url}:`,
          parseError,
        );
        console.error(
        ...
        console.error(
          ...
          trimmedContent.substring(Math.max(0, trimmedContent.length - 200)),
        );
        return [];
      }
    }
    ...
    if (!Array.isArray(suggestions)) {
      console.error(
        `Invalid suggestions format for ${url}:`,
        typeof suggestions,
      );
      return [];
    }
