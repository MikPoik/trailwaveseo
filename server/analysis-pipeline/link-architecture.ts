...
function generateLinkRecommendations(
  internalLinkStructure: LinkDistributionAnalysis,
  anchorTextAnalysis: AnchorTextAnalysis,
  linkEquityDistribution: LinkEquityAnalysis,
  pages?: PageAnalysisResult[]
  ...
  // Orphaned pages
  if (internalLinkStructure.orphanPages.length > 0) {
    recommendations.push({
      ...
      impact: 8,
      affectedPages: internalLinkStructure.orphanPages
    });
  }

  // Anchor text optimization
  if (anchorTextAnalysis.genericAnchors > 30) {
    recommendations.push({
  ...
  if (linkEquityDistribution.linkEquityDistribution < 50) {
    recommendations.push({
